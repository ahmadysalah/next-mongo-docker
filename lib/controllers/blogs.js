...
import { Assets, Blogs, Authors } from '../models';
...
class BlogController {
    ...
    static async createBlog(body) {
        ...
        const assets = await Assets.find({ _id: { $in: convertToObjectId(value.images) } });
        if (assets.length !== value.images.length) {
            const missingAssetIds = value.images.filter((id) =>
                !assets.some((asset) => asset._id.toString() === id));
            throw new Error(`One or more assets not found: ${missingAssetIds.join(', ')}, Please create the assets first`);
        }
